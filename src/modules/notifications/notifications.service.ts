...
export class NotificationsService {
  ...
  async send(
    ...
    try {
      ...
      this.logger.error(
        'NotificationsService.send() failed',
        err instanceof Error ? err.stack : String(err),
        `ticket_id=${ticketId} action=${actionName}`,
      );
    }
  }
  ...
  private async _doSend(
    ...
    if (!ticket) {
      this.logger.warn(
        'NotificationsService: ticket not found',
        `ticket_id=${ticketId} action=${actionName}`,
      );
      return;
    }
    ...
    if (!action) {
      this.logger.warn(
        'NotificationsService: action not seeded',
        `ticket_id=${ticketId} action=${actionName}`,
      );
      return;
    }
  ...
  async resolveTemplate(
    ...
    const catActionResponse =
      await this.prisma.category_action_responses.findFirst({
  ...
  private async sendToRecipient(
    ...
    try {
      ...
      this.logger.log(
        ...
        `ticket_id=${ticketId} action=${actionName} recipient=${email}`,
      );
