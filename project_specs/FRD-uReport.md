...
5. Append `duplicate` action to `ticketHistory` of **parent** ticket only, with `data = {duplicate: child_ticket_id}`. The child ticket's record of the duplication event is its closure entry (`action = 'closed'`, `substatus = Duplicate`).
...
| Public | Yes (OIDC login) | `NULL` (DB null, the column default) |
| Staff | Yes (OIDC login) | `'staff'` |

The `people.role` column is **nullable with no default** (`role TEXT NULL`). A DB-NULL value and an explicit `null` are treated identically as the `public` role. A request with a valid session but `people.role IS NULL` is treated as `public`. New users provisioned via OIDC callback receive no explicit `role` assignment — the NULL default applies, granting them `public` access until a staff administrator sets `role = 'staff'`.
...
- Triggered by `Accept: text/plain` header or `.txt` URL suffix.
- `Content-Type: text/plain; charset=utf-8` header set on response.
- TXT format parity is verified by snapshot tests against the legacy PHP output for the same input fixture (same as JSON/XML/CSV).
...
- **Digest subscription storage:** A user subscribes to digests by having a row in the `bookmarks` table with `type = 'digest'` and `person_id` = their people ID. The digest cron queries `bookmarks WHERE type = 'digest'` to enumerate subscribed users. No additional table or column is required.
...
- `active` (boolean, optional, default `true`): whether the client is authorized to use the API

**Validation:**
- `api_key` must be unique across all clients; duplicate → 409.
- `contactPerson_id` must reference an existing person.

**Revocation:** Setting `active = false` revokes the client's API access immediately. The `api_key` lookup in `POST /open311/v2/requests` filters `WHERE clients.active = TRUE`; a deactivated client receives HTTP 403. This allows revocation even when the client record cannot be deleted (due to FK references from tickets).

**Delete constraint:** Clients referenced by tickets (`tickets.client_id`) cannot be deleted; return 409. Use `active = false` to revoke access without deletion.
...
**Staleness:** Metrics are computed via **live SQL aggregations on every request** (no caching layer). This satisfies the ≤5-minute staleness requirement (NFR) with actual staleness of ~0ms. The ≤200ms response time NFR (NFR-6) is met by ensuring the following indexes exist on the `tickets` table: `tickets.status`, `tickets.enteredDate`, `tickets.category_id` (and via JOIN, `categories.department_id`).

**Process:**
1. Verify caller has `staff` role.
2. Compute metrics via live queries:
